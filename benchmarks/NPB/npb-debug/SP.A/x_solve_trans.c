...
void x_solve()
{
  ...
  for (k = 1; k <= nz2; k++) {
    ...
    for (j = 1; j <= ny2; j++) {
      ...
      for (i = 1; i <= nx2; i++) {
        lhs[j][i][0] =  0.0;
        lhs[j][i][1] = -(long double)dttx2 * (long double)cv[i-1] - (long double)dttx1 * (long double)rhon[i-1];
        lhs[j][i][2] =  1.0 + (long double)c2dttx1 * (long double)rhon[i];
        lhs[j][i][3] =  (long double)dttx2 * (long double)cv[i+1] - (long double)dttx1 * (long double)rhon[i+1];
        lhs[j][i][4] =  0.0;
      }
    }
    ...
    for (j = 1; j <= ny2; j++) {
      ...
      lhs[j][i+1][1] = lhs[j][i+1][1] - (long double)comz4;
      lhs[j][i+1][2] = lhs[j][i+1][2] + (long double)comz6;
      lhs[j][i+1][3] = lhs[j][i+1][3] - (long double)comz4;
      lhs[j][i+1][4] = lhs[j][i+1][4] + (long double)comz1;
    }
    ...
    for (j = 1; j <= ny2; j++) {
      ...
      lhs[j][i+1][0] = lhs[j][i+1][0] + (long double)comz1;
      lhs[j][i+1][1] = lhs[j][i+1][1] - (long double)comz4;
      lhs[j][i+1][2] = lhs[j][i+1][2] + (long double)comz5;
    }
    ...
    for (j = 1; j <= ny2; j++) {
      for (i = 1; i <= nx2; i++) {
        ...
        lhsp[j][i][1] = lhs[j][i][1] - (long double)dttx2 * speed[k][j][i-1];
        lhsp[j][i][2] = lhs[j][i][2];
        lhsp[j][i][3] = lhs[j][i][3] + (long double)dttx2 * speed[k][j][i+1];
        lhsp[j][i][4] = lhs[j][i][4];
        lhsm[j][i][0] = lhs[j][i][0];
        lhsm[j][i][1] = lhs[j][i][1] + (long double)dttx2 * speed[k][j][i-1];
        lhsm[j][i][2] = lhs[j][i][2];
        lhsm[j][i][3] = lhs[j][i][3] - (long double)dttx2 * speed[k][j][i+1];
