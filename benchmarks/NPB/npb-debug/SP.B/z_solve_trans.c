...
void z_solve()
{
  ...
  for (j = 1; j <= ny2; j++) {
    ...
    for (i = 1; i <= nx2; i++) {
      ...
      for (k = 1; k <= nz2; k++) {
        lhs[k][i][0] =  0.0;
        lhs[k][i][1] = -(long double)dttz2 * (long double)cv[k-1] - (long double)dttz1 * (long double)rhos[k-1];
        lhs[k][i][2] =  1.0 + (long double)c2dttz1 * (long double)rhos[k];
        lhs[k][i][3] =  (long double)dttz2 * (long double)cv[k+1] - (long double)dttz1 * (long double)rhos[k+1];
        lhs[k][i][4] =  0.0;
      }
    }
    ...
    for (i = 1; i <= nx2; i++) {
      k = 1;
      lhs[k][i][2] = lhs[k][i][2] + (long double)comz5;
      lhs[k][i][3] = lhs[k][i][3] - (long double)comz4;
      lhs[k][i][4] = lhs[k][i][4] + (long double)comz1;

      k = 2;
      lhs[k][i][1] = lhs[k][i][1] - (long double)comz4;
      lhs[k][i][2] = lhs[k][i][2] + (long double)comz6;
      lhs[k][i][3] = lhs[k][i][3] - (long double)comz4;
      lhs[k][i][4] = lhs[k][i][4] + (long double)comz1;
    }
    ...
    for (k = 3; k <= nz2-2; k++) {
      for (i = 1; i <= nx2; i++) {
        lhs[k][i][0] = lhs[k][i][0] + (long double)comz1;
        lhs[k][i][1] = lhs[k][i][1] - (long double)comz4;
        lhs[k][i][2] = lhs[k][i][2] + (long double)comz6;
        lhs[k][i][3] = lhs[k][i][3] - (long double)comz4;
        lhs[k][i][4] = lhs[k][i][4] + (long double)comz1;
    ...
    for (k = 1; k <= nz2; k++) {
      for (i = 1; i <= nx2; i++) {
        ...
        lhsp[k][i][1] = lhs[k][i][1] - (long double)dttz2 * speed[k-1][j][i];
        lhsp[k][i][2] = lhs[k][i][2];
        lhsp[k][i][3] = lhs[k][i][3] + (long double)dttz2 * speed[k+1][j][i];
        lhsp[k][i][4] = lhs[k][i][4];
        lhsm[k][i][0] = lhs[k][i][0];
        lhsm[k][i][1] = lhs[k][i][1] + (long double)dttz2 * speed[k-1][j][i];
        lhsm[k][i][2] = lhs[k][i][2];
        lhsm[k][i][3] = lhs[k][i][3] - (long double)dttz2 * speed[k+1][j][i];
