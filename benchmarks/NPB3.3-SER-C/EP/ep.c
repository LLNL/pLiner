...
{
  double tmp1;
  long double Mops, t1, t2, t3, t4, x1, x2;
  long double sx, sy, tm, an, tt, gc;
  long double sx_verify_value, sy_verify_value, sx_err, sy_err;
  int    np;
  int    i, ik, kk, l, k, nit;
  int    k_offset, j;
  ...
  sprintf(size, "%15.0lf", powl(2.0, M+1));
  j = 14;
  if (size[j] == '.') j--;
  size[j+1] = '\0';
  ...
  {dum[0] = randlc(&dum[1], (long double)dum[2]);
}  for (i = 0; i < 2 * NK; i++) {
    x[i] = -1.0e99;
  }
  Mops = logl(sqrtl(fabsl(MAX(1.0, 1.0))));   
  ...
  t1 = A;
  {tmp1=t1;vranlc(0, &tmp1, A, x);t1=tmp1;}
...
t1=tmp1;}  }
  ...
  for (k = 1; k <= np; k++) {
    ...
    for (i = 1; i <= 100; i++) {
      ik = kk / 2;
      if ((2 * ik) != kk) {tmp1=t1;t3 = randlc(&tmp1, t2);
t1=tmp1;}      if (ik == 0) break;
      {tmp1=t2;t3 = randlc(&tmp1, t2);
t2=tmp1;}      kk = ik;
    }
    ...
    {tmp1=t1;vranlc(2 * NK, &tmp1, A, x);t1=tmp1;}
    ...
    for (i = 0; i < NK; i++) {
      x1 = 2.0 * (long double)x[2*i] - 1.0;
      x2 = 2.0 * (long double)x[2*i+1] - 1.0;
      t1 = x1 * x1 + x2 * x2;
      if (t1 <= 1.0) {
        t2   = sqrtl(-2.0 * logl(t1) / t1);
        t3   = (x1 * t2);
        t4   = (x2 * t2);
        l    = MAX(fabs(t3), fabs(t4));
        q[l] = (long double)q[l] + 1.0;
        sx   = sx + t3;
        sy   = sy + t4;
      }
    }
  ...
  if (verified) {
    sx_err = fabsl((sx - sx_verify_value) / sx_verify_value);
    sy_err = fabsl((sy - sy_verify_value) / sy_verify_value);
    verified = ((sx_err <= EPSILON) && (sy_err <= EPSILON));
  }

  Mops = powl(2.0, M+1) / tm / 1000000.0;
