...
int main()
{
  ...
  long double rnm2, rnmu, old2, oldu, epsilon;
  int n1, n2, n3, nit;
  long double nn, verify_value, err;
  logical verified;
  ...
  //{tmp1=rnm2;tmp2=rnmu;norm2u3(v, n1, n2, n3, &tmp1, &tmp2, nx[lt], ny[lt], nz[lt]);rnm2=tmp1;rnmu=tmp2;}
  {tmp1=0;tmp2=0;norm2u3(v, n1, n2, n3, &tmp1, &tmp2, nx[lt], ny[lt], nz[lt]);rnm2=tmp1;rnmu=tmp2;}
  ...
  {tmp1=rnm2;tmp2=rnmu;norm2u3(r, n1, n2, n3, &tmp1, &tmp2, nx[lt], ny[lt], nz[lt]);rnm2=tmp1;rnmu=tmp2;}
  old2 = rnm2;
  oldu = rnmu;
  ...
  if (Class != 'U') {
    ...
    err = fabsl( rnm2 - verify_value ) / verify_value;
    ...
    for (i2 = 1; i2 < n2-1; i2++) {
      ...
      for (i1 = 1; i1 < n1-1; i1++) {
        u[i3][i2][i1] = u[i3][i2][i1]
                      + (long double)c[0] * r[i3][i2][i1]
                      + (long double)c[1] * ( r[i3][i2][i1-1] + r[i3][i2][i1+1]
                               + (long double)r1[i1] )
                      + (long double)c[2] * ( (long double)r2[i1] + (long double)r1[i1-1] + (long double)r1[i1+1] );
        //--------------------------------------------------------------------
  ...
  for (i3 = 1; i3 < n3-1; i3++) {
    for (i2 = 1; i2 < n2-1; i2++) {
      ...
      for (i1 = 1; i1 < n1-1; i1++) {
        ...
        //                     + u1[i1] )
        //-------------------------------------------------------------------
                      - (long double)a[2] * ( (long double)u2[i1] + (long double)u1[i1-1] + (long double)u1[i1+1] )
                      - (long double)a[3] * ( (long double)u2[i1-1] + (long double)u2[i1+1] );
      }
    }
  }
    ...
    for (j2 = 1; j2 < m2j-1; j2++) {
      ...
      for (j1 = 1; j1 < m1j-1; j1++) {
        ...
        s[j3][j2][j1] =
              ...
              + 0.25 * (r[i3+1][i2+1][i1] + r[i3+1][i2+1][i1+2] + (long double)x2)
              + 0.125 * ((long double)x1[i1] + (long double)x1[i1+2] + (long double)y2)
              + 0.0625 * ((long double)y1[i1] + (long double)y1[i1+2]);
      }
    }
  }
    ...
    for (i3 = 0; i3 < mm3-1; i3++) {
      for (i2 = 0; i2 < mm2-1; i2++) {
        for (i1 = 0; i1 < mm1; i1++) {
          ...
          z3[i1] = z[i3+1][i2+1][i1] + z[i3+1][i2][i1] + (long double)z1[i1];
        }
        ...
        for (i1 = 0; i1 < mm1-1; i1++) {
          u[2*i3][2*i2+1][2*i1] = u[2*i3][2*i2+1][2*i1]
                                + 0.5 * (long double)z1[i1];
          u[2*i3][2*i2+1][2*i1+1] = u[2*i3][2*i2+1][2*i1+1]
                                  + 0.25 * ((long double)z1[i1] + (long double)z1[i1+1]);
        }
        for (i1 = 0; i1 < mm1-1; i1++) {
          u[2*i3+1][2*i2][2*i1] = u[2*i3+1][2*i2][2*i1]
                                  + 0.5 * (long double)z2[i1];
          u[2*i3+1][2*i2][2*i1+1] = u[2*i3+1][2*i2][2*i1+1]
                                  + 0.25 * ((long double)z2[i1] + (long double)z2[i1+1]);
        }
        for (i1 = 0; i1 < mm1-1; i1++) {
          u[2*i3+1][2*i2+1][2*i1] = u[2*i3+1][2*i2+1][2*i1]
                                  + 0.25 * (long double)z3[i1];
          u[2*i3+1][2*i2+1][2*i1+1] = u[2*i3+1][2*i2+1][2*i1+1]
                                    + 0.125 * ((long double)z3[i1] + (long double)z3[i1+1]);
        }
      }
    }
  } else {
...
static void rep_nrm(void *u, int n1, int n2, int n3, char *title, int kk)
{
  ...
  //{tmp1=rnm2;tmp2=rnmu;norm2u3(u, n1, n2, n3, &tmp1, &tmp2, nx[kk], ny[kk], nz[kk]);rnm2=tmp1;rnmu=tmp2;}
  {tmp1=0;tmp2=0;norm2u3(u, n1, n2, n3, &tmp1, &tmp2, nx[kk], ny[kk], nz[kk]);rnm2=tmp1;rnmu=tmp2;}
...
static void zran3(void *oz, int n1, int n2, int n3, int nx, int ny, int k)
{
  ...
  long double xx, x0, x1, a1, a2, ai;

  const int mm = 10;
  const long double a = powl(5.0, 13.0);
  const long double x = 314159265.0;
  ...
  e3 = ie3 - is3 + 2;
  x0 = x;
  {tmp1=x0;rdummy = randlc(&tmp1, ai);
x0=tmp1;}
  for (i3 = 1; i3 < e3; i3++) {
    x1 = x0;
    for (i2 = 1; i2 < e2; i2++) {
      xx = x1;
      {tmp1=xx;vranlc(d1, &tmp1, a, &(z[i3][i2][1]));xx=tmp1;}
      {tmp1=x1;rdummy = randlc(&tmp1,a1);
x1=tmp1;}    }
    {tmp1=x0;rdummy = randlc(&tmp1, a2);
x0=tmp1;}  }
...
static double power(double a, int n)
{
  ...
  while (nj != 0) {
    if ((nj % 2) == 1) {tmp1=power;rdummy = randlc(&tmp1, aj);
power=tmp1;}    {tmp1=aj;rdummy = randlc(&tmp1, aj);
aj=tmp1;}    nj = nj/2;
  }
